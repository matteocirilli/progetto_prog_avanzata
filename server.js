...
function alreadyMail(req, res, next) {
    if (req.session.email)
        res.status(400).json({ error: 'la mail è già stata inserita' });
    else
        next();
}
...
function selectStorico(req, res, next) {
    if (!req.query.formato || !req.query.dataInizio || !req.query.dataFine)
        res.status(400).json({ error: 'mancano uno o più parametri tra: dataInizio, dataFine e formato' });
...
function checkAbbandona(req, res, next) {
    if (!req.session.idMatch)
        res.status(400).json({ error: "nessuna partita da abbandonare" });
    else
        next();
}
...
async function checkPartita(req, res, next) {
    ...
    const partita = partite.find(p => p.emailSfidante1 === req.session.personalEmail || p.emailSfidante2 === req.session.personalEmail);
    if (!partita) {
        res.status(400).json({ error: "non c'è nessuna partita in corso" });
    }
    else {
        if (partita.active === false) {
            res.status(400).json({ error: "la partita deve ancora iniziare" });
    ...
    if (req.user.admin) {
        if (req.body) {
            if (req.body.email) {
                ...
                if (!user)
                    res.status(400).json({ error: 'non esiste un utente con questa mail' });
                else
                    next();
            }
            else
                res.status(400).json({ error: 'devi inserire la mail' });
        }
        else
            res.status(400).json({ error: 'devi inserire il body' });
    }
    else
...
async function verifyToken(req, res, next) {
    if (!req.body.token) {
        res.status(400).json({ error: 'inserisci il numero di token da aggiungere' });
    }
    else if (!(Number.isInteger(req.body.token) && req.body.token > 0))
        res.status(400).json({ error: 'il numero non è corretto' });
    else {
        next();
...
app.get('/mossa', checkToken, verifyAndAuthenticate, async function (req, res) {
    ...
    if (!req.session.email) {
        res.status(400).json({ error: "inserisci la mail del tuo avversario!" });
    }
    else {
        ...
        if (!flag)
            res.status(400).json({ error: "aspetta il tuo avversario!" });
        else {
            ...
            if ((turni[partitaa.id] === 1 && req.session.personalEmail === partitaa.emailSfidante2) || (turni[partitaa.id] === 2 && req.session.personalEmail === partitaa.emailSfidante1)) {
                ...
                if (partiteLocal[req.session.idMatch][livello - 1][index] == 1) {
                    ...
                    res.status(400).json({ error: "la posizione è già occupata!" });
                    occupata = true;
                }
                if (!occupata) {
                    if (req.session.personalEmail === partitaa.emailSfidante1) {
